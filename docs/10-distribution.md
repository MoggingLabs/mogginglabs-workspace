...
| **Swept (208 gates)** | local (windows-sweep CI = Phase-6/03) | `macos-sweep` nightly + dispatch | `linux-sweep` nightly + dispatch |
...
† **Intel (x64) macOS is still deferred as of v0.17.0.** The 2026-07 macos runner image
...
gh release download v0.17.0 -D /tmp/rel && node scripts/update-manifests.mjs /tmp/rel
```
