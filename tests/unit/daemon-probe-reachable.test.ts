...
// corpse. The reused-pid corpse is untouched by the fix and is asserted here as the other
// direction.
...
const endpoint = (address: string): DaemonEndpoint => ({
  address,
  pid: process.pid,
...
describe('probeReachable', () => {
  ...
  it('still reports a corpse when nothing is listening (the reused-pid case is not weakened)', async () => {
