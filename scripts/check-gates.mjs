...
// src/main/index.ts, and named in its SMOKE_ENV allowlist.
//
// Three lists have to agree and nothing made them:
//   scripts/qa-smokes.sh   run_smoke NAME MOGGING_X …   (the sweep runs it)
//   src/main/index.ts      process.env.MOGGING_X        (the app runs the smoke)
//   src/main/index.ts      SMOKE_ENV = [… 'MOGGING_X' …] (the app knows it IS a smoke)
//
// Drift in the third one is the quiet kind. `isSmoke` decides whether the app skips the
...
if (!smokeEnv) {
  console.error('\nGATE REGISTRY: could not find SMOKE_ENV in src/main/index.ts — the pattern is blind.\n')
  process.exit(1)
}
...
if (undispatched.length || unlisted.length) {
  ...
  for (const g of undispatched) console.error(`  ${g.name.padEnd(16)} ${g.env}  not dispatched in src/main/index.ts`)
