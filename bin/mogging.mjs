...
// and authenticate, so `from` alone is a claim rather than a fact. Approval requires both
// credentials and is unavailable outside the pane that is signing off.
...
function runApprove(args) {
  ...
  if (!token) {
    process.stderr.write('mogging approve: pane credentials unavailable\n')
    process.exit(2)
  }
  withDaemon(
    (welcome, api) => {
      api.send({ t: 'approve', branch, from, token })
    },
    (m, api) => {
      if (m.t === 'approved') {
