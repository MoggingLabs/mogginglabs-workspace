...
| 06 | `06-websession-tier-and-adr.md` | **DONE** (2026-07-06): ADR 0007.b (4 clauses) + the `web-session` class (15 rows) — paste via the write-only 0007.a store, store-read PER-PROVIDER opt-in/OFF gated (spy proves the store is untouched when off), sensitive-origin blocklist refuses bank/mail/gov, never agent-facing; consent stub in Settings names the keychain touch; WEBUSAGE gate green (offOk/onOk/paste/cipher/replace/grep/sensitive/no-getter) |
| 07 | `07-cost-spend-and-history.md` | **DONE** (2026-07-06): local cost scan (Codex/Claude JSONL shapes dev-verified on real logs; price-table estimate w/ cache multipliers, unknown-model tokens counted honestly; 30-day/400-file bounded, on-demand only) + `spend?` on PlanUsage (fake fixture #11) + KV history ring (96 pts per provider×window, fed by the poller's own fresh samples) + `usage:cost`/`usage:history`; real-machine check: 7 days of plausible per-day spend, ~1.9s scan; USAGE +8 assertions, no new gate |
| 08 | `08-provider-status-feed.md` | **DONE** (2026-07-06): status feed on the seam — 5 dev-verified public statuspage endpoints (claude/openai/cursor/elevenlabs/deepgram; the REAL Claude+OpenAI incidents live that day normalized `degraded` w/ the pages' own wording), enabled-only sequential pass on one jittered 5m clock w/ backoff + hidden-pause; outage relabels a failing tile "provider outage" (pace muted) + status chip + one-glyph icon overlay; `usage:status`+push; catalog refuses non-https statusUrl; USAGE +12 assertions, no new gate |
| 09 | `09-profiles-plans-and-alerts.md` | **DONE** (2026-07-06): plans × profiles fan-out (`perProfile` adapters read every lane; per-lane stale), popover severity ordering + the rail's identity treatment on the active (order-0) lane, Enter/click switch = ONE `switchActive` (Phase-4 `profiles:save` order-swap + port announce; the failover-toast action is trigger #2), threshold engine (quiet 80/warn 95, single-fire per (provider,profile,window-epoch), KV-persisted, verdict line VERBATIM — DOM===IPC asserted), reset re-arm + quiet "fresh window" + OPT-IN confetti, failover suggestion only for active-lane-warn + idle sibling; Settings § Usage alert editor; USAGE +13 / USAGEUI +9 assertions, no new gate |
