...
| 8 | `phase-8/` | authored (remade 2026-07-06) | Integrations, five directions: ADR 0008 (protocols, not plugins; 8 stances incl. the custody rule), control plane joins the shipped MCP server (reads + granted writes), the agent web profile (phase-10 Comet resolution, Branch C) + the activity trail (FINDINGS §4.5), MCP manager + curated catalog honoring the WEBSITE roster (n8n + Google Workspace first; research: `docs/research/`), paste-once vault keys materialized into pane envs (the 0007.a grammar fleet-wide), per-workspace TOOL PLANS (context hygiene: which servers each CLI gets, chosen at creation, launch-time materialization, the who-has-what matrix), the outbound event bridge ("a notify call to any webhook"), a live MCP connection registry (per-server×CLI states, pane chips, restart nudges, one-click re-auth), GitHub adapter with review-lands-back-in-the-pane; 13 steps, 12 gates |
