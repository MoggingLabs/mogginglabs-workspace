...
| 8 | `phase-8/` | authored (remade 2026-07-06) | Integrations, five directions: ADR 0008 (protocols, not plugins; 7 stances), control plane joins the shipped MCP server (reads + granted writes), the agent web profile (phase-10 Comet resolution, Branch C) + the activity trail (FINDINGS §4.5), MCP manager + curated catalog honoring the WEBSITE roster (n8n + Google Workspace first; research: `docs/research/`), paste-once vault keys materialized into pane envs (the 0007.a grammar fleet-wide), the outbound event bridge ("a notify call to any webhook"), a live MCP connection registry (per-server×CLI states, pane chips, restart nudges, one-click re-auth), GitHub adapter with review-lands-back-in-the-pane; 12 steps, 11 gates |
