...
| 8.5 | `phase-8.5/` | authored (2026-07-08) | UI/UX revamp: one full audit (AUDIT.md — every surface graded keep/fix/remove) then a systemic visual pass — spacing scale + layout primitives in the token system, the wizard collapsed to ONE uncluttered page, a real clickable folder browser (breadcrumb + repo badges) beside the path bar, the Settings shell + both dense tabs (Integrations/Usage) rebuilt on cards with progressive disclosure, Home/first-run/board/palette/toasts/chrome unified into one feedback language, audit removals executed; 21st.dev as clean-room pattern research only (vanilla TS + house tokens, no new deps); 9 steps, 8 new gates, closed by UXMILESTONE with budgets unchanged + four-environment certification |
