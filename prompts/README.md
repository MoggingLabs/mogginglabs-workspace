...
| 11 | `phase-11/` | authored | Files: the explorer sidebar — a right-side dock with the workspace's folder open in a virtualized, git-decorated, live tree (ADR 0010 custody stance: a window, not a manager — read-only v1), FAR-RIGHT titlebar toggle + `Ctrl+Shift+E`, watch-what's-visible liveness (per-expanded-dir `fs.watch`, coalesced, zero new deps), per-file M/A/U/D/C badges riding the EXISTING git tick + the Changes lens, delegation-only actions (open/reveal/copy/send-to-pane) + the house context menu, `docs/16-files.md`; 7 steps, 7 new gates (sweep 76 → 83 as of authoring); numbering deconflicted with the holding phase-9 (which keeps docs/15 + ADR 0009); research: `phase-11/RESEARCH.md` (VS Code tree/decorations/watcher tiers, BridgeSpace's sidebar, tmux-sidebar virtues, the orchestrators' changed-files convergence) |
