Integrations start at the DECISION. One ADR codifies the eight stances
below; then the contracts every lane builds on. Ships ZERO runtime;
lanes fork.

## Steps
1. **ADR 0008 — integrations are protocols, not plugins** (`docs/adr/`),
   eight stances: (a) NO in-process plugin runtime (attacks rendering +
   ...
   user-configured webhooks — POST only, nothing listens, URLs are
   secrets, versioned documented payload of ids + the user's own note
   text, never scrollback/diffs/page content/origins; (h) **the custody
   rule**: any secret WE hold rests as OS-vault ciphertext — vault
   unavailable means refusal or session-only, NEVER plaintext (the 7/13
   posture); the CLIs' own stores are theirs (0002, exactly-as-terminal).
2. **The unified catalog** (`@contracts/integrations/mcp.ts` + JSON
   ...
   names/schemas verbatim + 5 control reads + 6 writes (the names:
   IMPLEMENTATION §01). Closed unions, no `any`; dispatch + docs derive
   from it; Phase 2.5 memory tools APPEND later.
3. **The unified grant**: `WorkspaceIntegrationsGrant { workspaceId,
   ...
   today's consent; migration maps the 6/05b boolean. Plus
   `SENSITIVE_ORIGIN_PATTERNS` (banking/mail/gov) that `actOrigins`
   never overrides — data here, enforced 03/04.
...
- `docs/adr/0008-integrations-protocols-not-plugins.md` ·
  `src/contracts/integrations/` · `src/contracts/index.ts` · greps

## Definition of Done
- ADR 0008 committed, eight stances, cross-refs (0002/0005/0007,
