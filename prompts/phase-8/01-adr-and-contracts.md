Integrations start at the DECISION, not the code. Codify "protocols, not plugins"
AND the browser-session boundary as one ADR, then the contracts seam
every lane builds on: ONE tool catalog as data (the shipped browser tools
join it), ONE grant shape (writes + web), the service-adapter interface.
Ships ZERO runtime; three lanes fork.
...
1. **ADR 0008 — integrations are protocols, not plugins** (`docs/adr/`), six
   ...
   the user's tools hold (`gh auth token`, in memory, once); service
   KEYS are env-ref POINTERS (ADR 0007, extended to services here);
   app-held OAuth is DEFERRED to its own ADR. (e) **Web sessions: consent-by-login only.** Agents act on
...
2. **The unified catalog** (`@contracts/integrations/mcp.ts`): `McpToolDef
   ...
   names/schemas verbatim (agents depend on them; reads `'read'`, act
   verbs `'act'`) + control reads (`list_panes`, `capture_pane`, `mail_read`,
...
3. **The unified grant** (same slice): `WorkspaceIntegrationsGrant {
   ...
   `'public'` = today's shipped consent; migration maps the 6/05b boolean. Plus `SENSITIVE_ORIGIN_PATTERNS` (banking/mail/gov) that
   `actOrigins` never overrides — data here, enforced in 03/04.
...
5. **Boundaries**: contracts depend on nothing; grep lines keep
   `@backend`/`@ui` on `@contracts/integrations` only.

## Files
- `docs/adr/0008-integrations-protocols-not-plugins.md` ·
  `src/contracts/integrations/` ·
  `src/contracts/index.ts` · boundary grep additions

## Definition of Done
- ADR 0008 committed, six stances explicit, cross-refs (0002/0005/07,
...
- Grant defaults live in the contract, not consumer code.
...
- Descriptions are for MODELS (02 serves them verbatim).
- `inputSchema` is plain JSON Schema data — contracts stay dependency-free.
- Verbs must already exist daemon/app-side; NEW capability is out of
  scope.
