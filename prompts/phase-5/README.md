...
and macOS, not just boot), the two deferred differentiators (a built-in browser pane
and voice input), real distribution (signing hooks, winget/homebrew manifests,
first-run experience), and a v0.4.0 milestone that proves it end to end. Same format
as `prompts/phase-1..4/` (each step self-contained + pasteable as a `/goal`). Execute
in order; each step file is < 4000 chars.

> Scope per `docs/02-mvp-and-roadmap.md`: the Phase-4 leftovers (**voice**, **built-in
> browser**) + the cross-cutting day-one commitments (signing, CI on three platforms).
...
| 04 | `04-voice-input.md` | Push-to-talk → local/system speech-to-text → reviewed text lands in the focused pane; mic consent explicit (smoke green) |
...
- A browser pane and voice input exist, budget-clean (both perf gates unchanged).
...
- No URLs, transcripts, audio, or update metadata in telemetry (counts/booleans only).
...
- Voice is LOCAL-FIRST: capture requires an explicit per-session consent, audio never
  leaves the machine, transcripts go only into the pane the user chose — never
  telemetry, logs, or disk.
- The daemon protocol stays at v3 — Phase 5 adds no wire surface.

## Parallelization
Lane A (platforms): 01 → 02. Lane B: 03 (browser). Lane C: 04 (voice). Lane D: 05
(first-run/updates). All independent after the pack starts; 06 needs A complete and
at least B or D (recommended: all) — it freezes the sweep and cuts v0.4.0.
