...
1. **Smoke** (`MOGGING_PRODUCT`, two-phase like every milestone):
   - **Phase A — the five-minute path, asserted.** Isolated FRESH boot → first-run
     ...
     pane opens beside the agents (03, localhost page served by the smoke) → voice
     path exercised with the fake STT engine (04: consent → chord → review → send →
     buffer) → `mogging mail/claim/approve` one-liners prove the swarm substrate is
...
2. **Full sweep, three platforms**: run `bash scripts/qa-smokes.sh` locally (Windows)
   ...
   VOICE, FIRSTRUN, PRODUCT — must pass on all three. Record the per-OS numbers in
...
3. **Release v0.4.0 — "Product"**: clean tree, bump, `npm run dist`, commit, push,
   `gh release create v0.4.0` with notes (browser pane, voice, first-run, update UX,
...
- `MOGGING_PRODUCT` green: installer-fresh state → guided setup → swarm + browser +
  voice, all in one asserted flow, budgets intact.
...
- No URLs, transcripts, card/mail text, paths, or hostnames in telemetry (final
  phase-wide grep, recorded in the pack README).

## Guardrails
- Never tag on a red or partial sweep — on ANY platform. If macOS/Linux CI is red,
  the release waits.
- Do NOT relax budgets to pass Phase B with the browser/voice surfaces on — they
  were built off the hot path; keep them there (01's CI-GPU mode exists ONLY for
  xvfb CI, never desktop).
- Deterministic shell provider everywhere; fake STT + local HTTP for the new
  features; vendor TUIs stay unasserted (OSC over hooks, hooks over parsing).
