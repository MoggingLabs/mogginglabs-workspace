...
1. **The full uncut local sweep** (all 172 gates). Not run here: the operator's
...
```bash
# 1. The full uncut local sweep (172 gates, ~4h). Run it on a QUIET machine:
...
# 2. The three-OS CI dispatch. `gates` EMPTY = all 172 gates; no ci.yml edit is
