...
1. **The full uncut local sweep** (all 201 gates). Not run here: the operator's
...
```bash
# 1. The full uncut local sweep (201 gates, ~4h). Run it on a QUIET machine:
...
# 2. The three-OS CI dispatch. `gates` EMPTY = all 201 gates; no ci.yml edit is
