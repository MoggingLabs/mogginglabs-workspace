...
1. **The full uncut local sweep** (all 134 gates). Not run here: the operator's
...
```bash
# 1. The full uncut local sweep (134 gates, ~4h). Run it on a QUIET machine:
...
# 2. The three-OS CI dispatch. `gates` EMPTY = all 134 gates; no ci.yml edit is
