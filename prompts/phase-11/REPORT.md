...
1. **The full uncut local sweep** (all 141 gates). Not run here: the operator's
...
```bash
# 1. The full uncut local sweep (141 gates, ~4h). Run it on a QUIET machine:
...
# 2. The three-OS CI dispatch. `gates` EMPTY = all 141 gates; no ci.yml edit is
