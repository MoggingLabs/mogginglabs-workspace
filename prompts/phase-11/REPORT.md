...
1. **The full uncut local sweep** (all 155 gates). Not run here: the operator's
...
```bash
# 1. The full uncut local sweep (155 gates, ~4h). Run it on a QUIET machine:
...
# 2. The three-OS CI dispatch. `gates` EMPTY = all 155 gates; no ci.yml edit is
