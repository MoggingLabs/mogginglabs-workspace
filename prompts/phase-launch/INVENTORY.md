...
| 199 | Alt-screen replay reconstruction | `src/backend/features/terminal/pane-modes.ts:426 composeReplay` | docs/adr/0006-detached-pty-daemon.md | ALTREPLAY | ~02 | ~05 | ~05 | ~05 | ~06 | ~05 |
| 200 | Grid heal on reveal and reconnect | `src/ui/features/terminal/terminal-pane.ts:1005 reassertGrid` | docs/01-architecture.md | GRIDHEAL | ~02 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| 201 | Pane soft-close undo grace | `src/ui/features/workspace/controller.ts:903 softClosePane` | docs/11-design-system.md | PANECLOSE | ~02 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| 189 | Attach-size reconciliation | `src/pty-daemon/attach-dims.ts:14 attachDims` | docs/adr/0006-detached-pty-daemon.md | REATTACHFIT,RESTOREDIMS | 02 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| # | Feature | Entry point | Spec | Gates | corr | smell | spag | dup | eff | debt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 66 | Agent account profiles | `src/main/profiles.ts:77 registerProfiles` | docs/09-swarm.md | PROFILES,PROFPERSIST_A,PROFPERSIST_B | ~03 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| 202 | Pane profile failover and manual switch | `src/ui/features/agents/index.ts:552 switchFocusedPane` | docs/09-swarm.md | PROFSWITCH | ~03 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| 203 | Usage alert outbox and capped truth | `src/backend/features/usage/alert-outbox.ts:31 CAPPED_BOUNDLESS_TTL_MS` | docs/12-usage.md | CAPFALSE | ~04 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| 204 | Device-code authorization ladder | `src/backend/features/integrations/oauth.ts:531 requestDeviceCode` | docs/adr/0014-app-held-service-connections.md | DEVICEFLOW | ~04 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| 205 | Installed CLI satellite closure | `src/main/cli-runtime.ts:11 SATELLITES` | docs/06-control-api.md | SATELLITE | ~04 | ~05 | ~05 | ~05 | ~06 | ~05 |
| 206 | CLI observable grammar and exit codes | `bin/lib/cli-core.mjs:63 parseInvocation` | docs/06-control-api.md | CLIGRAM | ~04 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
### defaults — Shared account defaults (ADR 0022)

| # | Feature | Entry point | Spec | Gates | corr | smell | spag | dup | eff | debt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 207 | Default-tier override store | `src/contracts/domain/agent-settings.ts:37 AGENT_CONFIG_ALL_ACCOUNTS` | docs/adr/0022-shared-account-defaults.md | DEFAULTSTORE | ~04 | ~05 | ~05 | ~05 | ~06 | ~05 |
| 208 | Default resolution and home fan-out | `src/backend/features/agent-settings/account-defaults.ts:34 resolveDefault` | docs/22-shared-defaults.md · docs/adr/0022-shared-account-defaults.md | PROFILEDEFAULTS,DEFAULTSMILESTONE | ~04 | ~05 | ~05 | ~05 | ~06 | ~05 |
| 209 | Applies-to control, consent and pins | `src/ui/features/settings/agent-config.ts:40 DEFAULTS_PROVIDERS` | docs/22-shared-defaults.md | DEFAULTSUX,DEFAULTSMILESTONE | ~04 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
| 210 | Vendored ConPTY overlay pin | `scripts/build-node-helper.mjs:82 CONPTY_PIN` | docs/01-architecture.md | CONPTYPIN | ~07 | ~05 | ~05 | ~05 | ~06 | ~05 |
| 211 | Doc citation accuracy | `scripts/check-docs-citations.mjs:33 LIVING_DOCS` | docs/02-mvp-and-roadmap.md | DOCSCITE | ~07 | ~05 | ~05 | ~05 | ~06 | ~05 |
| 212 | Smoke verdict integrity | `scripts/check-smoke-verdict.mjs:32 SENTINEL` | docs/05-perf-budget.md | VERDICT | ~07 | ~05 | ~05 | ~05 | ~06 | ~05 |
| 213 | Packaged artifact weight ceiling | `scripts/check-package-weight.mjs:54 MAX_MB` | docs/10-distribution.md | WEIGHT | ~07 | ~05 | ~05 | ~05 | ~06 | ~05 |
...
### A4 (2026-08-09, the port) — this tree's sweep joins the denominator

This file arrived here from the `mogging/c88ce15b` audit ref, where it closed a
**200**-gate sweep. This tree's sweep is **218** gates (180 app-boot + 38
static, `v0.18.0`) once `LAUNCHAUDIT` is registered in `scripts/qa-smokes.sh`,
and **17 of them were claimed by no row** — the sweep red exactly as check 1
promises.

Rows **199–213** now claim 16 of them; **RESTOREDIMS** rides row **189**
(attach-size reconciliation — it is the same dims invariant, measured across
three daemon generations), and **DEFAULTSMILESTONE** rides rows **208** and
**209**, the two surfaces it composes. One new area, **defaults** (ADR 0022),
had no rows at all.

Provenance is honest about what has and has not been swept: every new cell is
**pending**. The rendering/close rows are runtime & UI core (`~02`), the
profile-switch row is orchestration (`~03`), the usage/connections/cli/defaults
rows are money paths & reach (`~04`), and the four gate-script rows are
environment & failure (`~07`). None of these rows was audited by the branch —
they did not exist there — so a swept token on any of them would be a lie.

Two mechanical notes for whoever edits next:

- Row **66**'s entry point cited `src/main/profiles.ts:166` in a file that is
  now 136 lines; the anchor `registerProfiles` still resolves, at **:77**. The
  line was corrected, the symbol was not — which is exactly the split the
  columns table promises (line = signpost, symbol = anchor).
- The `ROWS:BEGIN` marker says "edit rows.txt and regenerate, never by hand."
  **There is no `rows.txt`** — it was a scratch artifact on the audit branch and
  never committed. These rows were added by hand. Either commit the generator or
  drop the marker; a law nobody can obey is worse than no law.
