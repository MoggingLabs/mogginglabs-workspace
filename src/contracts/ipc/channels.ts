...
export const BrainChannels = {
  ...
  recall: 'brain:recall', // (BrainRecallRequest) -> the serve layer's brain.recall reply (revision D: the launch seam's pre-brief door; hybrid only under the named workspace's consent)
  recallGet: 'brain:recallGet', // (workspaceId) -> boolean (default TRUE — the "what the team knows" section rides orientAtLaunch; active only when orient is ON)
  recallSet: 'brain:recallSet', // ({ workspaceId, on }) -> { ok } (the orient knob's storage shape, verbatim)
  memUsage: 'brain:memUsage', // (BrainRootRequest) -> BrainMemoryUsageAnswer (revision D: per-slug recall/read counts — the human's pruning truth, sortable in the view)
