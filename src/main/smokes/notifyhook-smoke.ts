...
export async function runNotifyHookSmoke(): Promise<void> {
  ...
  try {
    ...
    // ── channel rot: a STALE relay is rewritten on content drift, not kept for existing ──
    // The sink dir's channel segment is BAKED into the generated bytes, so a relay left
    // behind by a previous release keeps pushing every claude's numbers into the OLD
    // channel's dir while this build's monitor reads the new one — the statusline sink
