...
export async function startDaemonBackend(getWebContents: () => WebContents | null): Promise<() => void> {
    ...
    const c: DaemonClient = new DaemonClient(endpoint, {
      ...
      onWelcome: (panes) => {
        ...
        for (const p of panes) {
          ...
          const banked = specs.get(p.id)?.spec
    ...
    while (!disposed) {
      try {
        ...
        for (const [id, entry] of specs) {
          ...
          void next
            .spawn(id, entry.spec, undefined, replayMode)
            .then(async () => {
  ...
  ipcMain.handle(TerminalChannels.spawn, async (_e, req: SpawnRequest): Promise<SpawnResult> => {
    ...
    try {
      ...
      if (mine) {
        ...
        // NOT forgetPaneLaunch, deliberately — unlike the kill handler below. A failed spawn
        // does not FREE the id: the pane stays mounted under its dead banner, and the only
        // road back is restart, which re-spawns THIS pane. The id recycles when the pane is
        // CLOSED, and that goes through the kill handler, which forgets there. Dropping the
        // record here would only cost the restart its profile — back to order-0 on a pane
        // whose config home the user had chosen.
        client.kill(id)
      }
  ...
  ipcMain.on(TerminalChannels.resize, (_e, cmd: ResizeCommand) => {
    ...
    const entry = typeof gen === 'number' ? specs.get(id) : undefined
