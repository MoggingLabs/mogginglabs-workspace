...
import { runAltReplaySmoke } from './smokes/altreplay-smoke'
...
import { runBrainMilestoneSmoke } from './smokes/brainmilestone-smoke'
...
const SMOKE_ENV: readonly string[] = [
  ...
  'MOGGING_PANESCROLL', 'MOGGING_APPSCROLL', 'MOGGING_PANEFIT', 'MOGGING_GRIDHEAL', 'MOGGING_REATTACHFIT', 'MOGGING_RESTOREDIMS', 'MOGGING_ALTREPLAY',
...
async function beforeAppSettings(): Promise<boolean> {
  ...
  // Windowless alt-screen REPLAY smoke: the half of a reattach the ring cannot carry. A
  // head-trimmed ring loses the one `?1049h` a full-screen agent ever sends, so its frames
  // used to be replayed into a fresh xterm's NORMAL buffer — the missing-composer bug. The
  // replay now reconstructs the mode state and drops the frames (an alternate screen has no
  // history to preserve), and it does so WITHOUT putting a byte on the live channel.
  if (process.env.MOGGING_ALTREPLAY) {
    await runAltReplaySmoke()
    return true
  }
  ...
  // Windowless brain-parse smoke (ADR 0018.g): the WASM parser pool in the REAL
  // worker file + the grammar-catalog discipline (check red on tamper) — no daemon,
  // no window, zero UI, zero network, no graph.
  if (process.env.MOGGING_BRAINPARSE) {
    await runBrainParseSmoke()
    return true
  }

  // Windowless brain-graph smoke (ADR 0018 step 03): the deterministic build in the
  // worker — exact counts, byte-identical dump, worktree partitions, cache economics,
  // typed too-large/busy walls — no daemon, no window, zero UI, zero network.
  if (process.env.MOGGING_BRAINGRAPH) {
    await runBrainGraphSmoke()
    return true
  }
...
function afterWindow(win: BrowserWindow): void {
  ...
  } else if (process.env.MOGGING_MEMGRAPH) {
  ...
  } else if (process.env.MOGGING_BRAINMILESTONE) {
    runBrainMilestoneSmoke(win) // env-gated Phase-12 MILESTONE: the whole brain promise composed in ONE fixture world — shared-index economics across two worktrees, the oriented board launch, graph truth over real MCP, freshness under a real agent's write, granted atomic symbol writes + stale refusal, offline pinned docs, git-carried memory, the capture→promote→recall cipher arc with a labeled FAKE-embedder find, budgets held DURING a forced 5k-file re-index under 16 live panes, ADR 0005 hygiene, custody + zero sockets + the daemon protocol number equal (Phase-12/14)
