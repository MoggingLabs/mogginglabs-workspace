 ...
 * provider's OWN config-dir env vars FIRST, so an inherited one — set whenever a gate runs from
 * inside a Claude/Codex/Gemini/opencode session, i.e. the fleet/dogfood/CI-from-a-pane case —
 * steers the "isolated" gate straight into the user's REAL CLI config and mutates it (measured
 * 2026-07-15: SETAGENTCFG wrote a fixture, incl. a fake API key, into a live CLAUDE_CONFIG_DIR).
 * Delete the pointers so `pointer()` falls back to the isolated home.
 *
 * The list is DERIVED (`configPointerVars`, agent-config-pointers.ts — pure, with goldens),
 * because the hand-written one drifted from the registry it was meant to mirror: it named
 * gemini's legacy GEMINI_CONFIG_DIR and not the higher-precedence GEMINI_CLI_HOME, and
 * opencode's four direct reads were absent entirely.
 */
