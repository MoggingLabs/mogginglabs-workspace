...
// (ADR 0016 — the caller resolves it via node-helper.ts; the Electron binary itself is no
...
/** What spawning a daemon requires since the runtime split (ADR 0016): the standalone
...
export async function ensureDaemon(daemonEntry: string, host: DaemonHost): Promise<DaemonEndpoint> {
  ...
  // The standalone helper hosts the daemon (ADR 0016) — NOT process.execPath: the
