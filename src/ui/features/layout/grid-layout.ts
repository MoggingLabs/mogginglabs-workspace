...
export class GridLayout {
  ...
  // The in-flight drag's teardown (seam OR pane), removed only by its own `up` — so a
  // workspace closed MID-DRAG (a close-pane control command while the user holds a seam)
  // leaked the window mousemove/mouseup listeners, latched `body.resizing` forever, and left
  // a mouseup that could re-publish slots into a DETACHED grid — reviving pane ids (and their
  // PTYs) nobody can see. dispose() calls this to abort an in-flight drag.
