...
export function createUpdatesSection(): HTMLElement {
  ...
  const prerelease = createToggleRow({
    ...
    onChange: (on) => void save({ allowPrerelease: on }).then((ok) => { if (!ok) prerelease.setChecked(!on) })
  })
  ...
  const installOnQuit = createToggleRow({
    ...
    onChange: (on) => void save({ installOnQuit: on }).then((ok) => { if (!ok) installOnQuit.setChecked(!on) })
  })
